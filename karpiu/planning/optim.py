...
class ResponseMaximizer:
    ...
    def __init__(
        ...
        optim_coef_matrix = model.get_coef_matrix(
        ...
        if len(self.non_optim_regressor) > 0:
            coef_matrix = model.get_coef_matrix(
                date_array=calc_dt_array, regressors=self.non_optim_regressor
            )
            ...
            regressor_matrix = temp_df.loc[
                calc_dt_array, self.non_optim_regressor
            ].values
            ...
            reg_non_optim = np.sum(
                coef_matrix * np.log1p(adj_regressor_matrix / sat_array), -1
            )
            base_comp += reg_non_optim

        if len(self.event_regressor) > 0:
            coef_matrix = model.get_coef_matrix(
                date_array=calc_dt_array, regressors=self.event_regressor
            )
            coef_matrix = coef_matrix[self.n_max_adstock :, ...]
            temp_df = df.copy()
            temp_df = temp_df.set_index("dt")
...
class RevenueMaximizer:
    ...
    def __init__(
        ...
        transformed_bkg_matrix = adstock_process(
            bkg_spend_matrix, self.optim_adstock_matrix
        )
