...
class MMMShell:
    ...
    def __init__(
        ...
        self.input_mask = (
            (self.df[self.date_col] >= self.start) & 
        ...
        self.calc_mask = (
            (self.df[self.date_col] >= self.calc_start) & 
            (self.df [self.date_col] <= self.calc_end)
        )
        ...
        self.target_regressors = [x for x in self.full_regressors  if x in target_regressors]
        ...
        np.fill_diagonal(one_off_design_matrix, 0.)
