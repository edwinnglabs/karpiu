...
class CalibrationProcess:
    def __init__(
        ...
        init_report = CalibrationProcess.report_attribution(self.curr_mod, self.init_tests_df)
    ...
    def calibrate(
        ...
        self.logger.info("Total target distance: {}".format(self.calib_reports[-1]["target_dist"].sum()))
        ...
        while (iter < max_iters) and len(calibrating_channels) > 0:
            ...
            loc_prior = self._solve_channel_prior(curr_mod, target_channel, self.calib_reports[-1])
            ...
            new_report = CalibrationProcess.report_attribution(curr_mod, self.init_tests_df)
    ...
    @staticmethod
    def plot_cost_calib(calib_report_df: pd.DataFrame, calibrated_steps: pd.DataFrame, is_visible: bool = True):
        ...
        for idx, ch in enumerate(regressors):
            ...
            axes[idx].plot(x, y_target, linestyle="--", label="target", color="dodgerblue", alpha=0.5)
            axes[idx].fill_between(x, y_target_lower, y_target_upper, alpha=0.2, color="dodgerblue")

            mask = calibrated_steps['regressor'] == ch
            if np.any(mask):
                x_steps = calibrated_steps.loc[mask, 'iter']                
                axes[idx].scatter(x_steps, y_mmm[x_steps], color="orange", label="calibration")
    ...
    @staticmethod
    def plot_coef_calib(calib_report_df: pd.DataFrame, calibrated_steps: pd.DataFrame, is_visible: bool = True):
        regressors = calib_report_df["regressor"].unique().tolist()
        nrows = math.ceil(len(regressors) / 2)
        ...
        for idx, ch in enumerate(regressors):
            ...
            if np.any(mask):
                x_steps = calibrated_steps.loc[mask, 'iter']                
                axes[idx].scatter(x_steps, y_posteriors[x_steps], color="orange", label="calibration")
    ...
    def select_calib_channels(
        ...
        select_mask = (
            (report_df['spend'] > 1e-5) &
            (np.abs(report_df['mmm_icac'] - report_df['test_icac']) > self.se_tolerance * report_df['test_se'])
        ) 
        reduced_ch_df = (
            report_df
            .loc[select_mask, :]
    ...
    def append_model_info(
        ...
        expended_report = pd.concat([report_df, regression_summary], axis=1, join="inner")
    ...
    def report_attribution(
        ...
        for k in report_df.index:
            ...
            attributor = AttributorGamma(model, start=test_start, end=test_end, logger=logger)
            _, spend_attr, spend_df, _ = attributor.make_attribution()

            mask = (spend_attr[date_col] >= test_start) & (spend_attr[date_col] <= test_end)
        ...
        report_df["target_lift"] = np.round(report_df["spend"] / report_df["test_icac"], 0).astype(int)
        report_df["target_dist"] = np.abs(report_df["target_lift"] - report_df["mmm_attr"])
    ...
    def generate_prior(
        ...
        if modify_channel is not None:
            ...
            if sigma_prior is not None:
                ...
                new_priors_df.loc[modify_channel, 'scale_prior'] = max(loc_prior * 0.01, 1e-5)
        new_priors_df = new_priors_df.reset_index().rename(columns={'regressor': 'test_channel', 'loc_prior': 'coef_prior', 'scale_prior': 'sigma_prior'})
    ...
    def _solve_channel_prior(self, model, regressor, calib_report):
        ...
        def attr_obj_func(x, target):
            ...
            mask = (spend_attr[model.date_col] >= test_start) & (spend_attr[model.date_col] <= test_end)
    ...
    def get_calib_report(self) -> pd.DataFrame:
        return pd.concat([df.reset_index() for df in self.calib_reports], ignore_index=True)
