...
#         n_steps = self.target_regressor_matrix.shape[0]

#         # (n_steps, max_adstock + 1, n_regressors + 1)
#         delta_matrix = np.zeros((n_steps, self.max_adstock + 1, self.n_regressors + 1))
#         paid_on_attr_matrix = np.zeros_like(delta_matrix)
...
class Attributor:
    ...
    def make_attribution_numpy(
        ...
        full_attr_matrix = (
            norm_delta_matrix * 
            np.expand_dims(true_up_arr[2*max_adstock:len(true_up_arr) - max_adstock], (-1, -2))
        )
        ...
        if max_adstock > 0:
            ...
            useful_paid_on_attr_matrix = deepcopy(activities_attr_matrix)
