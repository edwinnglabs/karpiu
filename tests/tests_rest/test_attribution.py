...
def test_attribution_core(model_file):
    ...
    if max_adstock == 0:
        ...
        assert np.all(activities_attr_df[['organic']].sum() == spend_attr_df[['organic']].sum())
    ...
    new_activities_attr_df, new_spend_attr_df, new_spend_df, _ = new_attributor.make_attribution()
    ...
    new_avg_cost_of_sales = new_spend_df['search'].sum() / new_spend_attr_df['search'].sum()
